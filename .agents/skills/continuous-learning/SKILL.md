...
SessionEnd Hook 评估会话
    ↓
写入 $TAOZI_HOME/learned/（默认 ~/.taozi/learned/）
    ↓
...
每条学习记录保存在 `$TAOZI_HOME/learned/YYYY-MM-DD.json`。

默认目录：

- 新目录：`~/.taozi/learned/`
- 兼容旧目录：`~/.claude/taozi/learned/`
...
```bash
# 查看所有学习记录
ls "${TAOZI_HOME:-$HOME/.taozi}/learned/"

# 查看特定日期的记录
cat "${TAOZI_HOME:-$HOME/.taozi}/learned/2025-01-15.json"

# 搜索特定主题
grep -r "数据库" "${TAOZI_HOME:-$HOME/.taozi}/learned/"
```
